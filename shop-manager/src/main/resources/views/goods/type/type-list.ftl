      ...
      <div class="panel panel-default">
		...
		<div class="navbar navbar-default">
            <div class="row navbar-form">
            	...
            	<div class="col-xs-2">
                   <button type="submit" onclick="location.href='${ctx}/goods/type/add'"  class="btn btn-primary pull-right"><i class="fa fa-plus"></i>新增商品模型</button>
            	</div>
            </div>
          </div>
...
<#--										<a href="商品模型-2-属性列表.html" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="属性列表">属性列表</a>                                    -->
<#--                                        <a href="商品模型-3-规格列表.html" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="属性列表">规格列表</a>-->
...
<!-- /.content-wrapper -->
<script type="template" id="temp_01">
    {{ for(var i = 0; i < it.length; i++){ }}
    <tr>
        <td class="text-center">{{=it[i].id}}</td>
        <td class="text-center">{{=it[i].name}}</td>
        <td class="text-center">
            <a href="商品模型-2-属性列表.html" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="属性列表">属性列表</a>
            <a href="商品模型-3-规格列表.html" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="属性列表">规格列表</a>
            <a href="javascript:void(0);" onclick="uptype('{{=it[i].id}}','{{=it[i].name}}')" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="编辑"><i class="fa fa-pencil"></i></a>
            <a href="javascript:void(0);" onclick="delone('{{=it[i].id}}')" id="button-delete6" data-toggle="tooltip" title="" class="btn btn-danger" data-original-title="删除"><i class="fa fa-trash-o"></i></a>
        </td>
    </tr>
    {{ } }}
</script>
<!—3.模板调用 -->
<script type="text/javascript">
    var url="${ctx}/goods/type/query";
    $.ajax({
        url: url,
        type: "post",
        dataType: "json",
        success: function (data) {
            var template = doT.template($("#temp_01").text());
            $("#typelist").html(template(data));
        }
    })
</script>
  <script type="text/javascript">
      ...
      function uptype(id,name){
          checkgoodsTypeName(id);
      }
  </script>
</body>
</html>